...
class AAK(object):
    ...
    def __call__(self, M, mu, a, p0, e0, Y0, qS, phiS, qK, phiK, dist, Phi_phi0, Phi_theta0, Phi_r0, mich, dt=10, T=1):
        # print(dt,T)
        hS = self.Pn5AAK(M,
                         mu,
                         a,
...
class MBHB(object):
    ...
    def __call__(self, M, q, spin1z, spin2z, coa_phase, distance, iota, psi, t_c, T=1, dt=10):
        ...
        # bezier curve to smooth the the begining of the signal
        # fit using 1/32 period of the signal
        win = int(est_T / dt / 32)
        win_mid = bgn_idx_signal + zero_idx
        y1 = hp[win_mid - win:win_mid + win]
        y2 = hc[win_mid - win:win_mid + win]
        x = np.linspace(0, 1, num=2 * win)
        curve1 = bezier.Curve.from_nodes(np.vstack([x * dt, y1]))
        curve2 = bezier.Curve.from_nodes(np.vstack([x * dt, y2]))
        y1_hat = curve1.evaluate_multi(x)
        y2_hat = curve2.evaluate_multi(x)
        hp[win_mid - win:win_mid + win] = y1_hat[1, :]
        hc[win_mid - win:win_mid + win] = y2_hat[1, :]
    ...
    def m1_m2_from_M_Chirp_q(M_Chirp, q):
        ...
        eta = q / (1 + q)**2
        M = M_Chirp * eta**(-3 / 5)
        return BBH.m1_m2_from_M_q(M, 1 / q)
