...
class WaveformGen(object):
    ...
    def __init__(self, T=1, sample_rate=0.1, tdi_gen=2, det='Taiji', t0=10000, use_gpu=True):
        ...
        self.t0 = t0
        self.T = T     # [yr]
        self.time_duration = (int((T * Constant.YRSID_SI) / self.delta_t) + int(
            (T * Constant.YRSID_SI) / self.delta_t) % 2) * self.delta_t     # [s]
    ...
    def init_SOBBH(self,):
        ...
        self.sobbh_TDI = self.TDI(self.sobbh,
                                  index_lambda=2,
                                  index_beta=1,
                                  orbit_file=self.orbit_file,
                                  is_ecliptic_latitude=True,
